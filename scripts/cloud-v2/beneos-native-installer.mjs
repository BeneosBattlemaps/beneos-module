...
export class BeneosNativeBattlemapInstaller {
  ...
  async #verifyAndRepair(assets) {
    ...
    if (!this._isForge) {
      ...
      let geprueft = 0
      await this.#runPool(assets, DOWNLOAD_CONCURRENCY, async (a) => {
        // Der Fortschritt zaehlt JEDE Datei, auch die uebersprungene: die
        // Leiste misst den Durchlauf, nicht die Zahl der Pruefungen. Wer sie
        // an den Pruefungen aufhaengt, laesst sie bei jedem zu grossen Asset
        // vor dem Ende stehenbleiben.
        if (!byTarget.has(a.target) && !hopeless.has(a.target)) {
