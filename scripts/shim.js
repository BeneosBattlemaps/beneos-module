...
Hooks.once('init', () => {
    ...
    if(WARN_FALLBACK) {
        ...
        const [PACKAGE_ID, PACKAGE_TITLE] = (()=>{
            ...
            if(match[1] === 'systems') return dirs.find(n => n && game.system.id === n) ? [game.system.id, game.system.data?.title ?? game.system.title] : [null,null];
            const id = dirs.find(n => n && game.modules.has(n));
            return [id, game.modules.get(id)?.data?.title ?? game.modules.get(id)?.title];
        })();
