...
- Added: When Foundry cannot draw the canvas at all and every map stays black, Beneos now says so in plain words and names the packages Foundry found in the stack. It also explains the part that makes this look far worse than it is: once it happens, Foundry repeats the failure for the rest of the session, even after switching scenes, so reloading the page gives you a working canvas back. This kind of failure happens before any image is loaded, so it is never caused by the maps you installed.
- Fixed: On The Forge, a single large file could fail to install while everything else went through, and the report called it "unknown" and offered a retry that could never succeed. The real cause is the per-file upload limit of your Forge plan. The report now names it, quotes your own limit, drops the pointless retry, and points to the HD version of the release as the quick way forward. Your Forge limit is also part of the copied report now, so a question on Discord no longer starts with us asking which plan you are on.
